	...
	Setup_Map()
		...
		if(fexists("war.sav"))
			...
			for(var/z in 1 to world.maxz)
				...
				if(Minfo.oY <= 2 && Minfo.oX <= 6 && Minfo.oX >= 4)
					Minfo.village_control = "Konoha"
					if(Minfo.oY <= 1 && Minfo.oX <= 1 && Minfo.oX >= 2)
						Minfo.can_capture = 0
						Minfo.base_capture = 0
				else
					if(Minfo.oY >= 4 && Minfo.oY <= 6 && Minfo.oX <= 2)
						Minfo.village_control = "Suna"
						if(Minfo.oY >= 5 && Minfo.oY <= 6 && Minfo.oX <= 1)
							Minfo.can_capture = 0
							Minfo.base_capture = 0
					else
						if(Minfo.oX >= 8)
		...
		if(war)
			...
			for(var/mob/P in world)
				if(P.z == M.z && P.faction && P.faction.village == attacking_village)
					P.body += 1500*lp_mult_two
					P << "You gained [1500*lp_mult_two] Level Points!"
			for(var/faction/F)
				if(F.village == defending_village)
...
proc
	...
	Show_Score()
		if(!WAR) return
		for(var/A in Score)
			for(var/mob/human/O in world)
				if(O.client&&O.war)
					O<<"<span class='war'>[A]: [Score[A]]</span>"
	Start_War()
		if(WAR||chuuninactive) return
		spawn()
			Score = list()
			...
			if(WAR)
				...
				if(WAR)
					...
					if(!findtext(lead[1],",") && lead[1])
						world<<"<font size=+1><span class='war'>The War is Over! [lead[1]] Wins with [lead[2]] point(s).</span>"
						for(var/mob/human/M in world)
							if(M.war&&M.faction.village==lead[1])
								var/Money = rand(3000, 6000)
								M.body+=4000*lp_mult_two
								M.money+=Money
								M<<"You gained [4000*lp_mult_two] Level Points and [Money] dollars!"
					else
						if(WAR)
	...
	War
		verb
			Leave_War()
				set category="War"
				if(incombo) return
				winset(usr, "war_verb_leave", "parent=")
				if(pk)
					usr<<"You must be in a non PK zone to leave"
					return
				if(war)
					Player.Remove(usr)
					usr.war=0
					usr.verbs-=/mob/War/verb/Leave_War
					var/obj/Re=0
					for(var/obj/Respawn_Pt/R in world)
						if(R.ind==0)
							Re=R
...
mob
	proc
		join_war()
			...
			usr.oldz=usr.z
			Player.Add(usr)
			src.loc=locate_tag("war_room_[lowertext(src.faction.village)]")
			src.war=1
			for(var/mob/human/M in world)
				if(M.war)
