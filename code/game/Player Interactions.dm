mob/human/player
	verb
		...
		Whisper(mob/x in All_Clients(), var/msg as text)
			if(!x.client) return
			...
			if(usr.mute||usr.tempmute)
				...
				if(usr.name)
					...
					if(FilterText(msg,chat_filter))
						...
						world<<"<font color=green>[usr] has been auto-muted for trying to advertise in Whisper</b><font color>"
						usr.mute=2
						sleep(6000)
						usr.mute=0
					if(usr.talktimes>=8)
					...
					if(length(msg) <= 500&&usr.say==1)
						...
						usr.tempmute=0
mob
	verb/Disable_Whispers()
		if(usr)
			if(usr.block_whispers)
				usr.block_whispers = 0
				usr << "Your whisper has been toggled on."
				return
			usr.block_whispers = 1
			usr << "Your whisper has been toggled off."
mob
	verb/Disable_Arena()
		if(usr)
			if(usr.block_arena)
				usr.block_arena = 0
				usr << "Your arena has been enabled."
				return
			usr.block_arena = 1
			usr << "Your arena has been disabled."
...
mob
	proc
		Look_Map()
			var/obj/mapinfo/Minfo_2 =  locate("__mapinfo__[src.z]")
			var/obj/mapinfo/Minfo_ =  locate("__mapinfo__[src.z]")
			Get_Global_Coords()
